 ...
 * @Description: TODO
 * @Author Alex
 * @DateTime 2023/3/29 14:21
 * @updateDateTime 2023/3/29 14:21
 * @Version 1.0.0
 **/
@Component
public class RedisRepository {
    ...
    public boolean setObj(final String key, final Object obj, final long expire) {
        return redisTemplate.execute(new RedisCallback<Boolean>() {

            @Override
            public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
                connection.set(getSerializer().serialize(key), getSerializer().serialize(JSONUtil.toJsonString(obj)));
                return connection.expire(getSerializer().serialize(key), expire);
            }
        });
    }

    public boolean setNx(final String key, final Object obj, final long expire) {

        return redisTemplate.execute(new RedisCallback<Boolean>() {
            @Override
            public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
                connection.setNX(getSerializer().serialize(key), getSerializer().serialize(JSONUtil.toJsonString(obj)));
                return connection.expire(getSerializer().serialize(key), expire);
            }
        });
    }

    public boolean setNx(String key,String value,long minutes){
        Duration du1 = Duration.ofMinutes(minutes);
        return redisTemplate.opsForValue().setIfAbsent(key,value,du1);
    }
